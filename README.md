...
Как было сказано в разделе [История протокола HTTP](###ИсторияпротоколаHTTP), начиная с версии HTTP/2 сообщения данного протокола передаются в формате бинарных сообщений, однако при декодировании т.е переводе в текстовый формат, обыкновенно выглядят следующим образом:
